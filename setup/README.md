...
7. [Terraform deploy](terraform-deploy.md) - fill in `terraform.tfvars` with the four sensitive values (FTDv password, ISE password, reg key, NAT ID), run `init`/`plan`/`apply`, read the outputs, update Cloudflare with the real FTDv public IP.
